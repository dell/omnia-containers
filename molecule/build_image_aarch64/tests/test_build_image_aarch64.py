...
"""
Testinfra tests for build_image_aarch64 verification.

This file contains test functions that verify build_image_aarch64 deployment
was successful. Uses the same automation_library/build_image library as
build_image (x86_64) but passes arch='aarch64'.

Validations:
1. functional_groups_config.yml exists and contains all roles/groups from pxe_mapping
2. Base and compute images are available in regctl registry (rhel-aarch64_base)
3. All 3 images (initrd, rootfs, vmlinuz) are pushed to S3 bucket for each functional group

Usage:
    ./run_molecule.sh build_image_aarch64 test      # Run playbook + verify
    ./run_molecule.sh build_image_aarch64 verify    # Verify only
"""
