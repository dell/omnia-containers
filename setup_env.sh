...
if [ "$DEBUG" = true ]; then
    ...
    dbg "  molecule    : $(molecule --version 2>&1 | head -1 || echo 'not found')"
...
if [ -f "${MOLECULE_SCRIPT}" ]; then
    chmod +x "${MOLECULE_SCRIPT}"
    dbg "Made executable: ${MOLECULE_SCRIPT}"
fi
...
echo ""
printf "  ${C_CYAN}5.${C_RESET} Run molecule tests (two methods):\n"
echo ""
...
printf "     ${C_DIM}run_molecule --config                   ${C_RESET}${C_DIM}# run all enabled scenarios from config${C_RESET}\n"
echo ""
