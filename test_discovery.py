...
def test_nodes_ssh_reachable(host):
    ...
    Includes Kubernetes validation if kube control plane nodes are present.
    """
    ...
    for func_group, nodes in result.get("results_by_group", {}).items():
        ...
        for node in nodes:
            ...
            # Collect kube control plane nodes for K8s validation
            if "kube_control" in func_group.lower() and ssh_ok:
                kube_control_nodes.append({"hostname": hostname, "admin_ip": admin_ip})
        log.check("")

    # Kubernetes validation if kube control plane nodes are present
    if kube_control_nodes:
        log.check("═══ Kubernetes Nodes Validation ═══")
        log.check("Running 'kubectl get nodes -A' on all kube control plane nodes →")
        k8s_result = validate_kubernetes_nodes(host)
        
        if not k8s_result.get("skipped"):
            failed_cps = []
            for cp in k8s_result.get("control_plane_results", []):
            ...
            if k8s_result["success"]:
                total = sum(
                    ...
                    for cp in k8s_result["control_plane_results"]
                )
                log.check(f"  ✓ All K8s nodes Ready ({total} nodes)")
            else:
                log.check(f"  ✗ K8s issues on: {', '.join(failed_cps)}")
        else:
            log.check("  ✗ Kubernetes validation skipped")
        log.check("")
    else:
        log.check("═══ Kubernetes Nodes Validation ═══")
        log.check("  ✗ No kube control plane nodes in PXE mapping")
        log.check("")
    ...
    if result["success"]:
        ...
        # Add helpful message about checking node boot status
        failed_nodes_msg = f"Failed nodes: {', '.join(result['failed_nodes'])}"
        troubleshooting_msg = "💡 TROUBLESHOOTING TIP: Check if nodes completed PXE boot successfully"
        log.failed(
...
def test_all_services(host):
    ...
    log.check("")
    if result["success"]:
        log.passed("All Slurm services active on Slurm nodes", "sssd, munge, slurmd/slurmctld running on Slurm nodes")
    else:
