...
| --- | --- | --- | --- | --- | --- |
| Omnia Core | 1.0.0 | container | Apache-2.0 | OIM (`omnia_core` container) | Foundational container for cluster orchestration on the OIM. |
| OpenCHAMI | 0.5.3 | quadlet | Apache-2.0 | OIM (OpenCHAMI quadlets) | Open Composable Heterogeneous Adaptable Management Infrastructure for node discovery and lifecycle management. |
| SMD (State Manager Daemon) | 2.18.0 | quadlet | MIT | OIM (OpenCHAMI quadlets) | Monitors, tracks, and manages hardware components and node state. |
| BSS (Boot Script Service) | 1.32.1 | quadlet | MIT | OIM (OpenCHAMI quadlets) | Generates per-node boot scripts and provides Level 2 boot services for PXE/iPXE provisioning. |
| Image Builder | 0.1.2 | quadlet | MIT | OIM (OpenCHAMI quadlets) | Wrapper around buildah commands for layered image creation. |
| Magellan | 0.3.1 | quadlet | MIT | OIM (OpenCHAMI quadlets) | Redfish-based BMC discovery tool. |
| CoreSMD | 0.3.1 | quadlet | MIT | OIM (OpenCHAMI quadlets) | CoreDHCP plugin with a pull-through cache that communicates with SMD. |
| cloud-init | 24.4 | quadlet | MIT | OIM (OpenCHAMI quadlets) | Micro-service for serving cloud-init payloads during node provisioning. |
| HAProxy | 3.3-dev2 | quadlet | GPL-2.0-only | OIM (OpenCHAMI quadlets) | Reverse proxy for routing all OpenCHAMI microservices through a single HTTP(S) host. |
| Step-CA | 0.28.6 | quadlet | Apache-2.0 | OIM (OpenCHAMI quadlets) | Zero-trust certificate authority for X.509 certificate management. |
| Ory Hydra | 2.3.0 | quadlet | Apache-2.0 | OIM (OpenCHAMI quadlets) | OpenID Connect and OAuth 2.0 provider for service authentication. |
| Pulpcore | 3.80.1 | container | GPL-2.0-only | OIM (`pulp` container) | Repository management platform for mirroring RHEL and third-party repos. |
| pulp-cli | 0.33.0 | pip | GPL-2.0-only | OIM (`pulp` container) | Command-line interface for Pulp REST API operations. |
| pulp-deb | 3.5.2 | pip | GPL-2.0-only | OIM (`pulp` container) | Pulp plugin for managing Debian/Ubuntu package repositories. |
| pulp-cli-deb | 0.3.0 | pip | GPL-2.0-only | OIM (`pulp` container) | Pulp CLI extension for Debian repository commands. |
| pulp-glue | 0.33.0 | pip | GPL-2.0-only | OIM (`pulp` container) | Version-agnostic library for Pulp REST API communication. |
| pulp-glue-deb | 0.3.0 | pip | GPL-2.0-only | OIM (`pulp` container) | Pulp-glue extension for Debian repository operations. |
| Fedora CoreOS | 40 | image | MIT | OIM (base image) | Immutable, container-focused Linux distribution for provisioning base images. |
| MinIO | latest | quadlet | AGPLv3 | OIM (OpenCHAMI quadlets) | High-performance S3-compatible object storage for boot images and artifacts. |
| Docker Registry | latest | quadlet | Apache-2.0 | OIM (OpenCHAMI quadlets) | OCI image registry service for container image distribution. |
| PostgreSQL | 16.8 | container | PostgreSQL | OIM (`omnia_postgres` container) | Relational database management system used by OIM services. |
...
| Munge | 0.5.15 | rpm | GPL-2.0-only | `slurm_custom` (all Slurm nodes) | Authentication service for creating and validating user credentials across Slurm daemons. |
...
| OpenMPI | 5.0.8 | tarball | BSD-3-Clause | `openmpi` (Slurm nodes) | Open-source MPI implementation for distributed parallel computing. |
...
| Component | Version | Type | License | Installed On | Purpose |
| --- | --- | --- | --- | --- | --- |
| OpenLDAP | 2.6.9 | container | GPL-3.0-only | OIM (`omnia_auth` container) | Open-source LDAP directory server for centralized user authentication. |
| openldap-clients | 2.6.9 | rpm | OLDAP-2.8 | `openldap` (all cluster nodes) | Command-line tools for LDAP directory operations. |
| 389 Directory Server | 2.6.1 | container | GPL-3.0-or-later | OIM (`omnia_auth` container) | Lightweight Directory Access Protocol server. |
| FreeIPA | 4.12.2 | container | GPL-3.0-only | OIM (`omnia_auth` container) | Integrated identity management (alternative to standalone OpenLDAP). |
| Omnia Auth | 1.0.0 | container | Apache-2.0 | OIM (`omnia_auth` container) | Authentication services container for centralized access control with OpenLDAP integration. |

## Telemetry software stack

| Component | Version | Type | License | Installed On | Purpose |
| --- | --- | --- | --- | --- | --- |
| Strimzi Kafka | 0.48.0-kafka-4.1.0 | pod | Apache-2.0 | `service_k8s` (K8s pod) | Apache Kafka running on Kubernetes for telemetry data streaming. |
| Strimzi Kafka Operator | 0.48.0 | pod | Apache-2.0 | `service_k8s` (K8s pod) | Deploys and manages Apache Kafka clusters on Kubernetes. |
| Strimzi Kafka Operator Helm Chart | 0.48.1 | helm | Apache-2.0 | `service_k8s` (K8s pod) | Helm chart for deploying the Strimzi Kafka Operator. |
| Strimzi Kafka Bridge | 0.33.1 | pod | Apache-2.0 | `service_k8s` (K8s pod) | HTTP-based API for Apache Kafka, enabling REST clients to produce and consume messages. |
| VictoriaMetrics | 1.128.0 | pod | Apache-2.0 | `service_k8s` (K8s pod) | High-performance time-series database for metric storage. |
| vmagent | 1.128.0 | pod | Apache-2.0 | `service_k8s` (K8s pod) | Lightweight agent for collecting, filtering, and forwarding metrics to VictoriaMetrics. |
| vmstorage | 1.128.0 | pod | Apache-2.0 | `service_k8s` (K8s pod) | Storage node for VictoriaMetrics cluster mode. |
| vminsert | 1.128.0 | pod | Apache-2.0 | `service_k8s` (K8s pod) | Ingestion handler for VictoriaMetrics cluster mode. |
| vmselect | 1.128.0 | pod | Apache-2.0 | `service_k8s` (K8s pod) | Query execution handler for VictoriaMetrics cluster mode. |
| victoriapump | 1.0.0 | pod | Apache-2.0 | `service_k8s` (K8s pod) | Pushes telemetry metrics from the Omnia pipeline into VictoriaMetrics. |
| kafkapump | 1.0.0 | pod | Apache-2.0 | `service_k8s` (K8s pod) | Consumes telemetry data from Kafka topics and forwards it to downstream systems. |
...
| idrac-telemetry-receiver | 1.0.0 | pod | Apache-2.0 | `service_k8s` (K8s pod) | Collects and streams telemetry data from Dell iDRAC interfaces. |
| LDMS | 4.5.1 | rpm | GPL-2.0 | `ldms` (Slurm nodes) | Lightweight Distributed Metric Service for high-speed OS-level metric collection. |
| NERSC-LDMS | commit 1f46921 | helm | BSD-3-Clause | `service_k8s` (K8s pod) | Helm chart, image build, and dashboards for LDMS. |
| LDMS Aggregator (image) | 1.0.0 | pod | Apache-2.0, GPL-2.0 | `service_k8s` (K8s pod) | Ubuntu-based container with LDMS tools for telemetry and metric collection. |
| Prometheus | v3.4.1 | pod | Apache-2.0 | `service_k8s` (K8s pod) | Monitoring and alerting toolkit for metrics collection. |
...
| Component | Version | Type | License | Installed On | Purpose |
| --- | --- | --- | --- | --- | --- |
| containers.podman | 1.16.2 | pip | GPL-3.0-or-later | OIM (`omnia_core` container) | Ansible collection for Podman container management. |
| community.grafana | 2.1.0 | pip | GPL-3.0-only | OIM (`omnia_core` container) | Ansible collection for Grafana automation. |
| community.mysql | 3.10.3 | pip | GPL-3.0-only | OIM (`omnia_core` container) | Ansible collection for MySQL/MariaDB management. |
| kubernetes.core | 5.2.0 | pip | GPL-3.0-only | OIM (`omnia_core` container) | Ansible collection for Kubernetes and OpenShift cluster automation. |
| community.kubernetes | 2.0.1 | pip | GPL-3.0-or-later | OIM (`omnia_core` container) | Ansible collection for Kubernetes resources. |
| ansible-pylibssh | 1.2.3 | pip | LGPL-2.1-only | OIM (`omnia_core` container) | Python bindings for libssh specific to Ansible. |
| python3-netaddr | 0.8.0 | pip | BSD-2-Clause, BSD-3-Clause | OIM (`omnia_core` container) | Network address manipulation library for Python. |
| libssh | 0.10.6 | rpm | LGPL-2.1-or-later | OIM (`omnia_core` container) | SSH library for secure remote access. |
...
| Component | Version | Type | License | Installed On | Purpose |
| --- | --- | --- | --- | --- | --- |
| GitLab | Latest compatible | container | MIT | OIM (`omnia_build_stream` container) | CI/CD platform for BuildStreaM catalog-driven pipelines. |
| GitLab Runner | Latest compatible | container | MIT | OIM (`omnia_build_stream` container) | Executes CI/CD pipeline jobs dispatched by GitLab. |
| BuildStreaM Catalog | Bundled with Omnia | container | Apache-2.0 | OIM (`omnia_build_stream` container) | Declarative infrastructure catalog consumed by GitLab pipelines. |
