...
| Component | Version | Type | License | Installed On | Purpose |
| --- | --- | --- | --- | --- | --- |
| Kubernetes Core Components | 1.35.1 | rpm, pod | Apache-2.0 | service_kube_control_plane<br>service_kube_node | Control plane and node components: `kubectl`, `kubelet`, `kubeadm`, `kube-apiserver`, `kube-controller-manager`, `kube-scheduler`, `kube-proxy`, and `cri-o`. |
...
| MetalLB | v0.15.3 | pod | Apache-2.0 | service_kube_control_plane<br>service_kube_node | Bare-metal load balancer; assigns external IPs to `LoadBalancer` services. |
...
| Component | Version | Type | License | Installed On | Purpose |
| --- | --- | --- | --- | --- | --- |
| Slurm Workload Manager | 25.05.2 | rpm | GPL-2.0-only | slurm_control_node<br>slurm_node<br>login_node<br>login_compiler_node | HPC workload manager: job scheduling, resource allocation, and accounting. |
| Munge | 0.5.15 | rpm | GPL-2.0-only | slurm_control_node<br>slurm_node<br>login_node<br>login_compiler_node | Authentication service for creating and validating user credentials across Slurm daemons. |
...
| python3-PyMySQL | 1.1.2 | pip | MIT | slurm_control_node<br>service_kube_control_plane | Pure-Python MySQL client library for database connectivity. |
...
| Component | Version | Type | License | Installed On | Purpose |
| --- | --- | --- | --- | --- | --- |
| CUDA Toolkit | 13.0.2 | rpm | NVIDIA Software License | slurm_node<br>service_kube_node | Development environment for GPU-accelerated applications. |
| NVIDIA Container Runtime | 3.4.2 | rpm | Apache-2.0 | slurm_node<br>service_kube_node | Enables GPU access from within containers. |
...
| OFED | 24.10-1.1.4.0 | rpm | NVIDIA Software License | slurm_control_node<br>slurm_node<br>login_node<br>login_compiler_node<br>service_k8s | OpenFabrics Enterprise Distribution for InfiniBand and RDMA. |
...
| Component | Version | Type | License | Installed On | Purpose |
| --- | --- | --- | --- | --- | --- |
| Podman | 5.4.0 | rpm | Apache-2.0 | OIM<br>service_k8s | Daemonless container runtime used on the OIM and cluster nodes. |
...
| Component | Version | Type | License | Installed On | Purpose |
| --- | --- | --- | --- | --- | --- |
| containers.podman | 1.16.2 | ansible collection | GPL-3.0-or-later | omnia_core | Ansible collection for Podman container management. |
| community.grafana | 2.1.0 | ansible collection | GPL-3.0-only | omnia_core | Ansible collection for Grafana automation. |
| community.mysql | 3.10.3 | ansible collection | GPL-3.0-only | omnia_core | Ansible collection for MySQL/MariaDB management. |
| kubernetes.core | 5.2.0 | ansible collection | GPL-3.0-only | omnia_core | Ansible collection for Kubernetes and OpenShift cluster automation. |
| community.kubernetes | 2.0.1 | ansible collection | GPL-3.0-or-later | omnia_core | Ansible collection for Kubernetes resources. |
| ansible-pylibssh | 1.2.3 | pip | LGPL-2.1-only | omnia_core | Python bindings for libssh specific to Ansible. |
| python3-netaddr | 0.8.0 | pip | BSD-2-Clause, BSD-3-Clause | omnia_core | Network address manipulation library for Python. |
| libssh | 0.10.6 | rpm | LGPL-2.1-or-later | omnia_core | SSH library for secure remote access. |
| python3.12 | 3.12 | rpm | PSF | omnia_core<br>service_kube_control_plane | Python interpreter and standard library. |
