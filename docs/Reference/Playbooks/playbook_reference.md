...
| <b>upgrade/prepare_upgrade.yml</b> | Run after `omnia.sh --upgrade`: transforms 2.1.0.0 input files to the 2.2.0.0-rc1 format, restores and re-encrypts credentials from backup, and displays a summary of migrated files and new fields requiring review before running `upgrade.yml`. |
