...
| <b>discovery/discovery.yml</b> | Discovers bare-metal nodes via the specified `discovery_mechanism` (currently `ome`, with `magellan` planned) and registers them using Dell OpenManage Enterprise (OME) as the discovery source. |
