...
Each node must be equipped with active Network Interface Card:
...
After successfully running the ``provision.yml`` playbook, PXE boot the nodes to load diskless images from the OIM. For detailed steps on using ``set_pxe_boot.yml``, see :ref:`set-pxe-boot-order`.
