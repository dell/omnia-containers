...
??? note "Cause"
    ...
    - Incorrect or expired iDRAC credentials in the Telemetry vault (`bmc_username` / `bmc_password`), resulting in `401 Unauthorized` errors.
    ...
    - Collection target misconfiguration (`telemetry_sources.idrac.collection_targets` does not include both `kafka` and `victoria_metrics`).
...
??? note "Resolution"
    ...
    **Resolution steps:**
    ...
    3. Update iDRAC firmware to a version that supports Redfish EventService/Telemetry, then rerun Telemetry deployment.
    ...
    5. Set `telemetry_sources.idrac.collection_targets` to include both
       `victoria_metrics` and `kafka`, then rerun Telemetry deployment.
...
??? note "Cause"
    ...
    An ungraceful MySQL shutdown can leave stale `.sock` or `.pid` files on its
    persistent volume. The current `idrac-telemetry` StatefulSet includes the
    `cleanup-mysql-locks` init container to remove these two file types before
    MySQL starts. A continued crash after that initialization completes may
    indicate a storage attachment problem or database corruption rather than a
    stale runtime file.

??? note "Resolution"

    1. Inspect the initialization and MySQL logs:
    ...
    2. Confirm that `mysqldb-pvc-idrac-telemetry-0` is Bound and can attach to
       the node selected for the pod.

    3. If logs report corruption, preserve the PVC and engage the storage and
