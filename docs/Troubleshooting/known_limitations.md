...
Review this page before planning your deployment to understand the current limitations and constraints of Omnia 2.2.0.1.
...
- Omnia supports in-place upgrades only from **2.1.0.0** to **2.2.0.1**. Direct upgrades that skip releases (for example, **2.0.0.0** to **2.2.0.1**) are not supported. Upgrade one version at a time.
