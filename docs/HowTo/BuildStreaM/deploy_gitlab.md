...
* Ensure that Omnia BuildStreaM container, PostgreSQL container, and Playbook Watcher service are deployed on the OIM node (see [Step 3: Prepare the OIM](../../GetStarted/buildstream_deployment.md#step-3-prepare-the-omnia-infrastructure-manager))
...
After completing GitLab deployment, update the catalog file to automatically trigger the build pipeline. See [Execute Build Pipeline](update_catalog_pipeline.md).
