...
**Domain**
:   A specialized, independent component in Omnia v2.3's domain-based architecture that handles a specific aspect of cluster deployment. The 7 domains are: discovery (BMC discovery and PXE mapping), repo_manager (local repository creation), image_build_manager (OS image building), orchestrator (node provisioning and cluster setup), telemetry (monitoring pipeline), build_stream (GitOps CI/CD), and utils (utility operations). Domains communicate via YAML contracts and are executed through the omnia.sh CLI. See [Domain-Based Architecture](../index.md#domain-based-architecture).

**Domain Contract**
:   A YAML file that serves as the standardized communication interface between Omnia domains. Contracts define the data structures and parameters that domains exchange, enabling independent execution and reusability. For example, the orchestrator domain consumes contracts generated by the discovery and image_build_manager domains to provision nodes. See [Domain Execution](domain_execution.md).
...
**omnia.sh**
:   The unified command-line interface for Omnia v2.3 that replaces the container-based model from v2.2. The omnia.sh script provides a single entry point for executing domains, managing deployments, and interacting with the Omnia control plane. Supports domain-specific execution (e.g., `./omnia.sh --run discovery --tags execute`) and validation of configuration files. See [Prepare OIM](../HowTo/main/setup_oim.md).
