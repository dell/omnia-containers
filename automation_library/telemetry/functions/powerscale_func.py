...
def get_powerscale_deployment_mode(host) -> str:
    """
    Get PowerScale deployment mode from telemetry_config.yml.

    Returns:
        'omnia-orchestrated' or 'operator-provided'
    """
    ps_config = get_powerscale_config(host)
    return ps_config.get("deployment_mode", DEPLOYMENT_MODE_OMNIA)
...
def _get_vm_query_endpoint(host) -> Dict[str, Any]:
    """Get VictoriaMetrics query endpoint info based on deployment mode."""
    config = get_telemetry_config(host)
