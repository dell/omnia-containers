...
deployed alongside VictoriaMetrics when 'victoria' is in
...
def get_victoria_logs_config(host) -> Dict[str, Any]:
    """
    Get victoria_logs_configurations from telemetry_config.yml.

    Returns:
        Dict with victoria_logs_configurations (storage_size, retention_period)
    """
    config = get_telemetry_config(host)
    return config.get("victoria_logs_configurations", {})
...
def verify_victoria_logs_storage_size(host, admin_ip: str) -> Dict[str, Any]:
    """
    Verify VictoriaLogs vlstorage PVC sizes match victoria_logs_configurations.
